...
class ProductController extends Controller
{
    ...
    */      
    public function get_product_listing(Request $request){
        ...
        $sort         =($request->sort  == null) ? 'default' : $request->sort;
   ...
   /*
    ...
    |
    */ 
    public function get_product(Product $product, $productSlug)
    {
        $product=$this->get_product_query()->where('slug', $productSlug)->first();
    ...
    */  
    public function get_bestsellers(Request $request)
    {
        ...
        $query = DB::table('order')
                    ...
                    ->where('franchise', config('myapp.franchise'))
                    ->whereDate('created_date', '>', Carbon::now()->subDays(31))
        ...
        foreach ($query as $key => $product) {
            ...
            $product_query =$this->get_product_query()->where('product.id', $product->product)->first();
    ...
    |
    */  
    public function get_SpecialOffers(Request $request)
    {
        $limit =$request->limit;

        $query=$this->get_product_query()->where('product.old_price', '!=' , 0 );
    ...
    */ 
    public function get_featured_product(Request $request)
    {
        ...
        $query=$this->get_product_query()->where('product.featured', 1);
    ...
    |
    */ 
    public function get_TopRated(Request $request)
    {
        $limit =$request->limit;
        ...
        foreach ($top_rated as $key => $product) {
            $query=$this->get_product_query()->where('product.id', $product->id)->first();
            if(!empty($query)){
    ...
    |
    */ 
    public function get_new_arrivals(Request $request)
    {
        $limit        =$request->limit;
        $category_slug=$request->category;

        $query=$this->get_product_query()->orderBy('product.id', 'desc');
  ...
  /*
    ...
    */     
    public function get_all(Request $request){
        ...
        $query=$this->get_product_query()->get();
    ...
    */  
    public function get_category_by_slug($slug){
        ...
        return response()->json($this->get_category_obj($category));
...
public function get_customer_reviews(Request $request){
    ...
    $review = array(
        ...
        'text'=>   $newReview->customer_review,
    );
...
// public function get_navigation_ink_item(){
//     $item1 =array(
//         'label'=> 'Power Tools', 
//         'url'=> '/shop?category=health-n-beauty', 
//         );

//     $item =array(
//         'label'=> 'Power Tools', 
//         'url'=> '/shop?category=health-n-beauty', 
//         'items'=>array($item1,$item1,$item1,$item1,$item1) 
//         );
...
public function get_navigation_ink_item(){
    $parents = DB::table('product_category')
    ...
    foreach($parents as $key => $parent){
        $menu= array(
            ...
            'columns'=>  $this->get_sub_category_links($parent->id)
       );
...
private function get_sub_category_links($parent){
    $categories = DB::table('product_category')
    ...
    foreach($categories as $key=> $category){
        $item =array(
            ...
            'items'=> $this->get_grand_sub_category_links($category->id)
            );
    ...
    */  
    private function site_product($product)
    {
        ...
        if($product->meta_title){
            ...
            $product_title ='Foyo - '.$product->title;
        }
    ...
    public function get_productReview($id){
        ...
        $query= DB::table('customer_review')
                    ->where('customer_review.product', $id)
                    ->select('customer_review.customer_name','customer_review.rating','customer_review.customer_review','customer_review.created_at')   
                    ->get();
    ...
    */      
    private function get_category_obj($category_obj,$get_children=true){
        ...
        $category = array(
            "id"=> $category_obj->id,
            "type"=> "shop",
            "name"=> $category_obj->product_category,
            "slug"=> $category_obj->slug,
            "path"=> $category_obj->slug,
            "image"=> $image,
            "items"=> 272,
            "customFields"=> array(),
            "parents"=> array(),
        ...
        $parent = $this->get_category_by_id($category_obj->category_parent,false);
        if(!is_null($parent)){
            $category["parents"] = array($parent);
        }
